...
@st.cache_data
def generate_dispatch_chart(dunkelflaute_df, mitigation_mw):
    """
    Separate chart generation to improve performance on re-renders.
    """
    fig = go.Figure()

    # 1. Nuclear (Baseload - The Floor)
    fig.add_trace(go.Scatter(
        x=dunkelflaute_df.index, 
        y=dunkelflaute_df['Nuclear_Gen_2030_MW']/1000,
        mode='lines', 
    ...
    fig.add_trace(go.Scatter(
        x=dunkelflaute_df.index, 
        y=(dunkelflaute_df['Wind_Gen_2030_MW'] + dunkelflaute_df['Solar_Gen_2030_MW'])/1000,
        mode='lines', 
        name='Wind & Solar',
        stackgroup='one',
    ...
    fig.add_trace(go.Scatter(
        x=dunkelflaute_df.index, 
        y=dunkelflaute_df['Battery_Output_MW'].clip(lower=0)/1000,
        mode='lines', 
    ...
    # 4. Mitigation (Dynamic Tiers)
    if mitigation_mw > 0:
        mitigation_series = pd.Series(mitigation_mw/1000, index=dunkelflaute_df.index)
        fig.add_trace(go.Scatter(
            x=dunkelflaute_df.index,
            y=mitigation_series,
            mode='lines',
            name='Tiers 1-4 Mitigations',
            stackgroup='one',
    ...
    fig.add_trace(go.Scatter(
        x=dunkelflaute_df.index, 
        y=dunkelflaute_df['Demand_2030_MW']/1000,
        mode='lines', 
        name='System Demand',
...
with col1:
    strategy_card("Event Window", f"{start_date_str} - {end_date_str}", "Worst 7 Days")

with col2:
    strategy_card("Clean Surplus", f"{curtailment:,.1f} TWh", "Wasted Pre-Event")
...
# --- 5. The "Merit Order" Chart (Top - Restored Layout) ---
st.divider()
st.subheader("🔎 The Dispatch Stack")

# Use cached chart generator for performance
...
# --- 6. TABS (Bottom - Restored Layout) ---
st.divider()
st.markdown("### 📚 Context & Methodology")
tab_context, tab_method, tab_findings, tab_market, tab_refs = st.tabs([
    "❄️ The Weather Challenge", 
    "🧪 Methodology", 
    "🎯 Key Findings",
    "🏗️ Strategic Levers", 
    "📚 References"
...
with tab_context:
    ...
    with col_c1:
        ...
        **The Resilience Gap:**
        The Government's *Clean Power 2030* mission relies heavily on wind. This simulation models a **"Dunkelflaute"** event (German for "dark doldrums"), a prolonged period of low wind and minimal sunshine.
...
with tab_method:
    ...
    3.  **Dispatch Engine:** A custom Python engine (see codebase link in References) calculated the net deficit half-hour by half-hour.
    """)
...
with tab_findings:
    ...
    with col_f1:
        st.markdown("#### 1. The Energy Gap is Physical")
        st.write("""
        Even if we hit the most ambitious wind targets, a week-long lull creates a massive supply shortfall. 
        Building more turbines does not solve a lack of wind; during these events, generation crashes to <5%, 
        leaving a gap renewables alone cannot fill.
        """)
        
    with col_f2:
        st.markdown("#### 2. Batteries are Sprinters")
        st.write("""
        Battery storage is brilliant for balancing the grid for an hour or two, but it isn't a "marathon runner." 
        In our simulation, the UK's battery fleet is exhausted by Day 2 of a cold snap. 
        To keep the lights on for the remaining 5 days, we need "Long-Duration" solutions.
        """)
        
    with col_f3:
        st.markdown("#### 3. Interconnection is a Shared Risk")
        st.write("""
        We often assume we can just import power from Europe. However, severe weather systems often sit over 
        the entire North Sea at once. If our neighbours are freezing and calm too, 
        their surplus disappears, and the safety net of the sub-sea cables fails.
        """)
